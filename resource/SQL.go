...
func (f *FutureSQL) WorkDeleteByIds(pf *ExecutableFuture, req *FutureRequest, k *FutureRequestKindDeleteByIds) {
	...
	var params []SQLExpression
	id_sql_name := f.Resource.GetIdFieldName(nil)
	for _, id := range k.Ids {
		params = append(params, NewSQLEquals(id_sql_name, id))
	}
	query := &SQLQuery{
		Query:        "DELETE FROM %s WHERE ",
		FmtArguments: []interface{}{f.Resource.Table},
	}
	NewSQLOr(params...).Express(query)

	q := query.PrepareQuery()
	pf.Request.API.Logger.Debugf("QUERY: %s %#v\n", q, query.SqlArguments)
	_, err = tx.Exec(q, query.SqlArguments...)
	if err != nil {
		req.SendResponse(&FutureResponse{
			IsSuccess: false,
...
func (f *FutureSQL) Work(pf *ExecutableFuture) {
	for {
		...
		case *FutureRequestKindDeleteByIds:
			f.WorkDeleteByIds(pf, req, k)
		default:
